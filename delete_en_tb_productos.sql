...
#Se borrarán algunos de los registros que se registraron
...
# Selecciona los códigos que de la DB principal que no aparezcan en la DB secundaria. 
SELECT CODIGO FROM tb_producto WHERE CODIGO NOT IN (SELECT CODIGO_DEL_PRODUCTO FROM jugos_ventas.tabla_de_productos);

# Elimina los registros encontrados en el SELECT IN SELECT anterior
